...
Add the package repository first (see [documentation]).
...
To do that, you can use the [`docker-cleanup-volumes.sh` script].  It will
remove all the volumes that are not related to any containers.  Note that this
script must be launched with root permissions.
...
[`docker-cleanup-volumes.sh` script]: https://github.com/chadoe/docker-cleanup-volumes
